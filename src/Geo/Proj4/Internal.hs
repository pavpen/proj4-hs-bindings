...
module Geo.Proj4.Internal where

import Foreign (Storable (..), Ptr (..), FunPtr (..), alloca, castPtr,
                plusPtr, nullPtr)
import Foreign.C
--import GHC.Fl
--import GHC.Prim
...
-- | A @'FunPtr' for 'pj_free', useful for a 'ForeignPtr' finalizer.
foreign import ccall "proj_api.h &pj_free" p_c_pj_free ::
    FunPtr (OCIprojPJHdl -> IO ())
